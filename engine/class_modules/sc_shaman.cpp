...
void shaman_t::trigger_deeply_rooted_elements( const action_state_t* state )
{
  ...
  // Enhancement transitions to use a new formula that's modeled with accumulated_rng_t
  else if ( !rng_obj.dre_enhancement->trigger() )
  {
    return;
  }
...
void shaman_t::init_action_list_enhancement()
{
  ...
  //_Use_items
    ...
    single->add_action( "doom_winds,if=raid_event.adds.in>=action.doom_winds.cooldown&(buff.legacy_of_the_frost_witch.up|!talent.legacy_of_the_frost_witch.enabled)" );
    ...
    single->add_action( "ascendance,if=(dot.flame_shock.ticking|!talent.primordial_wave.enabled|!talent.ashen_catalyst.enabled)&(ti_lightning_bolt&active_enemies=1&raid_event.adds.in>=action.ascendance.cooldown%2)&buff.maelstrom_weapon.stack>=2" );
    ...
    single->add_action( "lightning_bolt,if=buff.maelstrom_weapon.stack>=9&!buff.primordial_storm.up&buff.arc_discharge.stack>0" );
    ...
    single->add_action( "lava_lash,if=talent.lashing_flames.enabled&(debuff.lashing_flames.down|dot.flame_shock.remains<5)" );
    single->add_action( "stormstrike,if=(talent.stormblast.enabled&charges=max_charges)&buff.stormblast.stack>1" );
    single->add_action( "lava_lash,if=(buff.hot_hand.up&(buff.ashen_catalyst.stack=buff.ashen_catalyst.max_stack))|(dot.flame_shock.remains<=2&!talent.voltaic_blaze.enabled)" );
    single->add_action( "crash_lightning,if=buff.electrostatic_wager.stack>4" );
    ...
    single_totemic->add_action( "doom_winds,if=raid_event.adds.in>=action.doom_winds.cooldown&!talent.elemental_spirits.enabled&buff.legacy_of_the_frost_witch.up&(cooldown.primordial_wave.remains>3|!talent.primordial_wave.enabled)" );
    ...
    single_totemic->add_action( "primordial_storm,if=(buff.maelstrom_weapon.stack>=10)&(buff.earthen_weapon.stack>=3|buff.doom_winds.remains<=3)&(buff.legacy_of_the_frost_witch.up|!talent.legacy_of_the_frost_witch.enabled)|(buff.primordial_storm.remains<=4&buff.maelstrom_weapon.stack>=7)" );
    ...
    single_totemic->add_action( "primordial_wave,if=dot.flame_shock.ticking&(raid_event.adds.in>action.primordial_wave.cooldown|raid_event.adds.in<6)&(cooldown.doom_winds.remains>3)" );
    ...
    single_totemic->add_action( "lava_lash,if=buff.whirling_fire.up&buff.legacy_of_the_frost_witch.up" );
    single_totemic->add_action( "elemental_blast,if=(buff.maelstrom_weapon.stack>=10)&!buff.primordial_storm.remains<=4" );
    single_totemic->add_action( "lightning_bolt,if=(buff.maelstrom_weapon.stack>=10)&!buff.primordial_storm.remains<=4" );
    ...
    aoe->add_action( "primordial_wave,if=(active_dot.flame_shock=6|(active_dot.flame_shock>=4&active_dot.flame_shock=active_enemies))" );
    ...
    aoe->add_action( "voltaic_blaze,if=active_dot.flame_shock<5" );
    aoe->add_action( "crash_lightning,if=talent.converging_storms.enabled&buff.electrostatic_wager.stack>3" );
    ...
    aoe_open->add_action( "primordial_wave,if=buff.maelstrom_weapon.stack>=4&(active_dot.flame_shock=6|(active_dot.flame_shock>=4&active_dot.flame_shock=active_enemies))" );
    ...
    aoe_totemic_open->add_action( "primordial_wave" );
    ...
    aoe_totemic_open->add_action( "lava_lash,if=buff.hot_hand.up|buff.legacy_of_the_frost_witch.up" );

...
public:
  ...
  struct pets_t
  {
    ...
    pets_t( death_knight_t* p )
      : ghoul_pet( "ghoul", p,
                   p->talent.unholy.raise_dead.ok() && !p->talent.sacrificial_pact.ok()
                       ? spawner::pet_spawn_type::PET_SPAWN_PERSISTENT
                       : spawner::pet_spawn_type::PET_SPAWN_DYNAMIC ),
        army_ghouls( "army_ghoul", p ),
        apoc_ghouls( "apoc_ghoul", p ),
        gargoyle( "gargoyle", p ),
        risen_skulker( "risen_skulker", p, spawner::pet_spawn_type::PET_SPAWN_PERSISTENT ),
...
struct death_knight_pet_t : public pet_t
{
      ...
      commander_value( 0 )
  {
    ...
    {
      decomposition_extend_limit = player->talent.unholy.decomposition->effectN( 3 ).time_value();
    }
...
struct ghoul_pet_t final : public base_ghoul_pet_t
{
  ...
  {
    sweeping_claws_t( ghoul_pet_t* p, std::string_view options_str )
...
struct army_ghoul_pet_t final : public base_ghoul_pet_t
{
  ...
  {
    army_claw_t( army_ghoul_pet_t* p, std::string_view options_str )
  ...
  army_ghoul_pet_t( death_knight_t* owner, std::string_view name = "army_ghoul" )
...
struct dancing_rune_weapon_pet_t : public death_knight_pet_t
{
  ...
  {
    soul_reaper_execute_t( std::string_view name, dancing_rune_weapon_pet_t* p )
  ...
  {
    soul_reaper_t( std::string_view n, dancing_rune_weapon_pet_t* p )
    ...
    dot_t* get_dot( player_t* t ) override
    {
      ...
      if ( dk()->buffs.reaper_of_souls->up() )
      {
    ...
    void tick( dot_t* dot ) override
    {
      if ( is_reaper_of_souls || dot->target->health_percentage() < data().effectN( 3 ).base_value() )
      {
        ...
        if ( dk()->buffs.dancing_rune_weapon->up() )
          ...
          dk()->background_actions.soul_reaper_execute_expired_drw->execute_on_target( dot->target );
        is_reaper_of_souls = false;
      }
    }
...
struct abomination_pet_t : public death_knight_pet_t
{
  struct disease_cloud_t : public pet_spell_t<abomination_pet_t>
  {
      ...
      : pet_spell_t( p, name, p->dk()->pet_spell.abomination_disease_cloud )
    {
      background    = true;
      aoe           = 0;
      impact_action = dk()->background_actions.virulent_plague;
...
{
  razorice_attack_t( std::string_view name, death_knight_t* player )
...
{
  unholy_pact_damage_t( std::string_view name, death_knight_t* p )
...
{
  decomposition_damage_t( std::string_view name, death_knight_t* p )
...
public:
  death_knight_absorb_buff_t( death_knight_t* p, std::string_view name, const spell_data_t* spell )
...
{
  decomposition_debuff_t( death_knight_td_t& td, std::string_view n, const spell_data_t* s )
  ...
  {
    damage = p()->background_actions.decomposition_damage;
...
{
  blood_plague_t( std::string_view name, death_knight_t* p, bool superstrain = false )
    ...
    : death_knight_disease_t( name, p, p->spell.frost_fever ),
      rp_generation(
          as<int>( p->spec.frost_fever->effectN( 1 ).trigger()->effectN( 1 ).resource( RESOURCE_RUNIC_POWER ) +
                   ( p->talent.unholy.superstrain->effectN( 3 ).base_value() * 0.1 ) ) )
  {
...
{
  unholy_blight_dot_t( std::string_view name, death_knight_t* p )
...
{
  unholy_blight_t( std::string_view n, death_knight_t* p )
...
struct summon_rider_t : public death_knight_summon_spell_t
{
  summon_rider_t( std::string_view name, death_knight_t* p, const spell_data_t* s, timespan_t duration = 10_s,
                  bool random = true )
    : death_knight_summon_spell_t( name, p, s ), duration( duration ), random( random )
  {
    background = true;
  }
...
{
  abomination_limb_damage_t( std::string_view n, death_knight_t* p )
...
struct abomination_limb_t : public death_knight_spell_t
{
    ...
    : death_knight_spell_t( "abomination_limb", p, p->talent.abomination_limb )
  {
    ...
    {
      tick_action        = p->background_actions.abomination_limb_damage;
...
{
  apocalypse_t( death_knight_t* p, std::string_view options_str )
...
// Raise Abomination =========================================================
struct raise_abomination_t final : public death_knight_summon_spell_t
{
  raise_abomination_t( death_knight_t* p, std::string_view options_str )
    : death_knight_summon_spell_t( "raise_abomination", p, p->talent.unholy.raise_abomination ),
      duration( 0_s ),
      rider_duration( 0_s ),
      precombat_time( 0_s )
  {
...
struct bonestorm_t final : public death_knight_spell_t
{
  bonestorm_t( death_knight_t* p, std::string_view options_str )
    : death_knight_spell_t( "bonestorm", p, p->talent.blood.bonestorm ), max_charges( 0 )
  {
    parse_options( options_str );
    hasted_ticks = false;
...
{
  cold_heart_damage_t( std::string_view name, death_knight_t* p )
...
struct chill_streak_damage_t final : public death_knight_spell_t
{
      ...
      current( 0 )
  {
    ...
    enduring_chill_chance = p->talent.frost.enduring_chill->effectN( 1 ).percent();
  }
...
{
  chill_streak_t( death_knight_t* p, std::string_view options_str )
...
{
  dark_transformation_damage_t( std::string_view name, death_knight_t* p )
...
struct dark_transformation_t final : public death_knight_spell_t
{
  ...
  dark_transformation_t( death_knight_t* p, std::string_view options_str )
    : death_knight_spell_t( "dark_transformation", p, p->talent.unholy.dark_transformation ), winning_streak_stacks( 0 )
  {
    harmful = false;
    target  = p;
    ...
    energy_gain = p->spec.dark_transformation_2->effectN( 1 ).base_value();
  ...
  void execute() override
  {
    ...
    if ( p()->spec.dark_transformation_2->ok() )
    {
      p()->pets.ghoul_pet.active_pet()->resource_gain(
          RESOURCE_ENERGY, energy_gain, p()->pets.ghoul_pet.active_pet()->dark_transformation_gain, this );
    }
...
struct death_and_decay_damage_base_t : public death_knight_spell_t
{
  death_and_decay_damage_base_t( std::string_view name, death_knight_t* p, const spell_data_t* spell )
    : death_knight_spell_t( name, p, spell ), pest_chance( 0 )
  {
    aoe        = -1;
    background = dual = true;
    tick_zero         = true;
    if ( p->talent.unholy.pestilence.ok() )
      pest_chance = p->talent.unholy.pestilence->effectN( 1 ).percent();
  }

  void impact( action_state_t* s ) override
  {
    death_knight_spell_t::impact( s );
    if ( p()->talent.unholy.pestilence.ok() && rng().roll( pest_chance ) )
    {
...
{
  defile_damage_t( std::string_view name, death_knight_t* p )
...
{
  defile_t( death_knight_t* p, std::string_view options_str )
...
{
  festering_wound_application_t( std::string_view n, death_knight_t* p )
  ...
  void impact( action_state_t* s ) override
  {
    ...
    if ( result_is_hit( s->result ) )
    {
      ...
      p()->trigger_festering_wound( s, n_stacks,
                                    this->data().id() == p()->spell.festering_scythe->id()
                                        ? p()->procs.fw_festering_scythe
                                        : p()->procs.fw_festering_strike );
    }
  }
...
{
  shattered_frost_t( std::string_view n, death_knight_t* p )
...
struct frost_strike_strike_t final : public death_knight_melee_attack_t
{
  frost_strike_strike_t( std::string_view n, death_knight_t* p, weapon_t* w, const spell_data_t* s,
    ...
    : death_knight_melee_attack_t( n, p, s ), sb( shattering_blade ), shattered_frost( nullptr )
  {
...
{
  horn_of_winter_t( death_knight_t* p, std::string_view options_str )
...
struct marrowrend_t final : public death_knight_melee_attack_t
{
  ...
  void execute() override
  {
    ...
    if ( p()->buffs.exterminate->up() )
    {
      ...
      make_event<delayed_execute_event_t>( *sim, p(), p()->background_actions.exterminate, execute_state->target, 500_ms );
    }
...
struct outbreak_t final : public death_knight_spell_t
{
  ...
  {
    parse_options( options_str );
    impact_action = p->background_actions.outbreak_aoe;
  }
};
...
{
  frostwhelps_aid_t( std::string_view name, death_knight_t* p )
...
struct raise_dead_t final : public death_knight_summon_spell_t
{
  raise_dead_t( death_knight_t* p, std::string_view options_str )
    : death_knight_summon_spell_t(
          "raise_dead", p, p->talent.unholy.raise_dead.ok() ? p->talent.unholy.raise_dead : p->talent.raise_dead )
  {
    parse_options( options_str );
...
{
  sacrificial_pact_damage_t( std::string_view name, death_knight_t* p )
...
{
  sacrificial_pact_t( death_knight_t* p, std::string_view options_str )
...
{
  scourge_strike_shadow_t( std::string_view name, death_knight_t* p )
...
{
  soul_reaper_execute_t( std::string_view name, death_knight_t* p )
...
{
  soul_reaper_action_t( std::string_view n, death_knight_t* p, std::string_view options_str )
...
struct summon_gargoyle_t final : public death_knight_summon_spell_t
{
  summon_gargoyle_t( death_knight_t* p, std::string_view options_str )
    : death_knight_summon_spell_t( "summon_gargoyle", p, p->talent.unholy.summon_gargoyle )
  {
    parse_options( options_str );
    harmful = false;
    target  = p;
...
{
  unholy_assault_t( death_knight_t* p, std::string_view options_str )
...
void runeforge::fallen_crusader( special_effect_t& effect )
{
  ...
  {
    fallen_crusader_heal_t( std::string_view name, death_knight_t* p, const spell_data_t* data )
...
void death_knight_t::trigger_festering_wound( const action_state_t* state, unsigned n, proc_t* proc )
{
  ...
  {
    proc->occur();
    background_actions.festering_wound_application->execute_on_target( state->target );
  }
}
...
void death_knight_t::trigger_bursting_sores( player_t* target, unsigned n )
{
  struct bs_event_t : public event_t
  {
    ...
    void execute() override
    {
      ...
      {
        if ( p()->talent.unholy.bursting_sores.ok() && p()->background_actions.bursting_sores->target_list().size() != 0 )
        {
          p()->background_actions.bursting_sores->execute_on_target( t );
        }
      }
    }
  };
  ...
  struct cs_bounce_t : public event_t
  {
    ...
    void execute() override
    {
      ...
      for ( const auto target : target_list )
      {
        if ( target != t )
        {
          dk->background_actions.chill_streak_damage->set_target( target );
          dk->background_actions.chill_streak_damage->schedule_execute();
          return;
        }
      }
    }
...
void death_knight_t::reapers_mark_explosion_wrapper( player_t* target, player_t* source, int stacks )
{
  ...
  {
    debug_cast<reapers_mark_explosion_t*>( background_actions.reapers_mark_explosion )->execute_wrapper( target, stacks );
  }
}
...
void death_knight_t::create_actions()
{
  ...
  {
    background_actions.abomination_limb_damage = get_action<abomination_limb_damage_t>( "abomination_limb_damage", this );
  }
  ...
  if ( specialization() == DEATH_KNIGHT_BLOOD )
  {
    ...
    if ( talent.soul_reaper.ok() )
    {
      background_actions.soul_reaper_execute_expired_drw =
  ...
  else if ( specialization() == DEATH_KNIGHT_UNHOLY )
  {
    ...
    {
      background_actions.festering_wound = get_action<festering_wound_t>( "festering_wound", this );
      background_actions.festering_wound_application =
          get_action<festering_wound_application_t>( "festering_wound_application", this );
    }

    if ( talent.unholy.bursting_sores.ok() )
    {
      background_actions.bursting_sores = get_action<bursting_sores_t>( "bursting_sores", this );
    }

    if ( talent.unholy.unholy_pact.ok() )
    {
      background_actions.unholy_pact_damage = get_action<unholy_pact_damage_t>( "unholy_pact_damage", this );
    }
    ...
    if ( talent.unholy.decomposition.ok() )
    {
      background_actions.decomposition_damage = get_action<decomposition_damage_t>( "decomposition", this );
    }

    if ( talent.unholy.unholy_blight.ok() )
    {
      background_actions.unholy_blight = get_action<unholy_blight_t>( "unholy_blight", this );
    }
  }
...
void death_knight_t::create_buffs()
{
  ...
  if ( this->specialization() == DEATH_KNIGHT_BLOOD )
  {
    ...
    buffs.unbroken_tww1_2pc =
        make_buff( this, "unbroken", spell.unbroken_tww1_2pc )
  ...
  buffs.remorseless_winter =
      make_fallback( spec.remorseless_winter->ok(), this, "remorseless_winter", spec.remorseless_winter )
          ...
          ->set_stack_change_callback( [ this ]( buff_t*, int, int new_ ) {
            ...
            {
              debug_cast<remorseless_winter_damage_t*>( background_actions.remorseless_winter_tick )->triggered_biting_cold =
                  false;
            }
          } );
      ...
      make_fallback( is_ptr() && sets->has_set_bonus( DEATH_KNIGHT_FROST, TWW2, B2 ), this, "winning_streak",
          ...
          ->set_expire_callback( [ this ]( buff_t*, int, timespan_t ) {
            if ( !sim->event_mgr.canceled && sets->has_set_bonus( DEATH_KNIGHT_FROST, TWW2, B4 ) )
            {
              ...
              double old_mult = background_actions.frostscythe_proc->base_multiplier;
              background_actions.frostscythe_proc->base_multiplier =
                  sets->set( DEATH_KNIGHT_FROST, TWW2, B4 )->effectN( 3 ).percent();
              background_actions.frostscythe_proc->execute();
              background_actions.frostscythe_proc->base_multiplier = old_mult;
            }
          } );
  ...
  buffs.unholy_pact =
      make_fallback( talent.unholy.unholy_pact.ok(), this, "unholy_pact",
          ...
          ->set_tick_callback( [ this ]( buff_t*, int, timespan_t ) { background_actions.unholy_pact_damage->execute(); } )
          ...
          ->set_expire_callback( [ this ]( buff_t*, int, timespan_t ) {
            background_actions.unholy_pact_damage->execute();
            background_actions.unholy_pact_damage->execute();
          } );
...
{
  //  TWW1 4pc handling
  if ( specialization() == DEATH_KNIGHT_BLOOD && sets->has_set_bonus( DEATH_KNIGHT_BLOOD, TWW1, B4 ) &&
       ...
       rng().roll( sets->set( DEATH_KNIGHT_BLOOD, TWW1, B4 )->effectN( 5 ).percent() ) )
  {
    if ( rng().roll( 0.5 ) )  // 50% chance to roll 1 or 2 charges
      buffs.piledriver_tww1_4pc->trigger(
          as<int>( sets->set( DEATH_KNIGHT_BLOOD, TWW1, B4 )->effectN( 3 ).base_value() ) );
    else
      buffs.piledriver_tww1_4pc->trigger(
          as<int>( sets->set( DEATH_KNIGHT_BLOOD, TWW1, B4 )->effectN( 4 ).base_value() ) );
  }
...
void death_knight_t::do_damage( action_state_t* state )
{
  ...
  if ( runeforge.rune_of_sanguination )
  {
    ...
    if ( background_actions.runeforge_sanguination->ready() )
      background_actions.runeforge_sanguination->execute();
  }

...
void beast_mastery( player_t* p )
{
  ...
  cleave->add_action( "barbed_shot,target_if=min:dot.barbed_shot.remains,if=set_bonus.thewarwithin_season_2_2pc|(talent.wild_call|talent.barbed_scales|talent.savagery)&charges_fractional>1.4|buff.call_of_the_wild.up|talent.scent_of_blood&(cooldown.bestial_wrath.remains<12+gcd)|fight_remains<9" );
  cleave->add_action( "explosive_shot,if=talent.thundering_hooves" );
  cleave->add_action( "lights_judgment,if=buff.bestial_wrath.down|target.time_to_die<5" );
  cleave->add_action( "cobra_shot,if=buff.hogstrider.stack>3" );
  ...
  st->add_action( "barbed_shot,target_if=min:dot.barbed_shot.remains,if=set_bonus.thewarwithin_season_2_2pc|(talent.wild_call|talent.barbed_scales|talent.savagery)&charges_fractional>1.4|buff.call_of_the_wild.up|talent.scent_of_blood&(cooldown.bestial_wrath.remains<12+gcd)|fight_remains<9" );

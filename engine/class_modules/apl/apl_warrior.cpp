...
void arms( player_t* p )
{
  ...
  colossus_aoe->add_action( "demolish,if=buff.colossal_might.stack=10&debuff.colossus_smash.remains>=2|cooldown.colossus_smash.remains>=7&buff.colossal_might.stack=10" );
  ...
  colossus_aoe->add_action( "execute,if=buff.sweeping_strikes.up&talent.executioners_precision" );
  ...
  slayer_execute->add_action( "mortal_strike,if=dot.rend.remains<2|debuff.executioners_precision.stack=2" );
  slayer_execute->add_action( "overpower,if=rage<=40&buff.martial_prowess.stack<2&talent.fierce_followthrough" );
  slayer_execute->add_action( "overpower,if=rage<=85&buff.martial_prowess.stack<2&buff.opportunist.up&talent.opportunist" );
  ...
  slayer_st->add_action( "overpower,if=buff.opportunist.up&buff.winning_streak_arms.stack>=1" );
  ...
  slayer_sweep->add_action( "overpower,if=buff.winning_streak_arms.stack>=7&charges=2" );
  ...
  trinkets->add_action( "use_item,slot=trinket2,if=!variable.trinket_2_buffs&(trinket.2.cast_time>0&!buff.avatar.up|!trinket.2.cast_time>0)&!variable.trinket_2_manual&(!variable.trinket_2_buffs&(trinket.1.cooldown.remains|!variable.trinket_1_buffs)|(trinket.2.cast_time>0&!buff.avatar.up|!trinket.2.cast_time>0)|cooldown.avatar.remains_expected>20)" );
  trinkets->add_action( "use_item,slot=main_hand,if=!equipped.fyralath_the_dreamrender&!equipped.bestinslots&(!variable.trinket_1_buffs|trinket.1.cooldown.remains)&(!variable.trinket_2_buffs|trinket.2.cooldown.remains)" );
  trinkets->add_action( "actions.trinkets+=/use_item,name=bestinslots,if=cooldown.avatar.remains>20|(buff.avatar.up&(!trinket.1.has_cooldown|!trinket.2.has_cooldown))" );

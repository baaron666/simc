...
void devastation( player_t* p )
{
  ...
  default_->add_action( "potion,if=buff.dragonrage.up&(!cooldown.shattering_star.up|debuff.shattering_star_debuff.up|active_enemies>=2)|fight_remains<35", "Delay pot in ST if you are about to SS - mostly relevant for opener where you want DR->FB->SS->rotation" );
  ...
  aoe->add_action( "living_flame,if=talent.snapfire&!talent.engulfing_blaze" );
  ...
  st->add_action( "firestorm,if=buff.snapfire.up&talent.scorching_embers|!debuff.in_firestorm.up&talent.feed_the_flames" );
  ...
  trinkets->add_action( "use_item,name=bestinslots,if=buff.dragonrage.up|!talent.dragonrage" );
  trinkets->add_action( "use_item,name=neural_synapse_enhancer,if=buff.dragonrage.up|cooldown.dragonrage.remains>=15|!talent.dragonrage" );

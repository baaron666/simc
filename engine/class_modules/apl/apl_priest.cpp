...
void shadow( player_t* p )
{
  ...
  main->add_action( "void_bolt,target_if=max:target.time_to_die,only_cwc=1" );
  main->add_action( "variable,name=pooling_mindblasts,op=setif,value=1,value_else=0,condition=(cooldown.void_torrent.remains<?(variable.holding_crash*raid_event.adds.in))<=gcd.max*(1+talent.mind_melt*1),if=talent.void_blast", "Are we pooling mindblasts? Currently only used for Voidweaver." );
  ...
  main->add_action( "void_torrent,target_if=max:(dot.devouring_plague.remains*1000+target.time_to_die),if=!variable.holding_crash&!talent.entropic_rift&(cooldown.mind_blast.full_recharge_time>=2&(!buff.voidform.up|!bugs)|(cooldown.mind_blast.full_recharge_time>=2|cooldown.void_bolt.remains<=2*gcd.max)&buff.voidform.up&bugs&cooldown.void_bolt.remains<=3-gcd.max),target_if=dot.devouring_plague.remains>=2.5", "Use Void Torrent if it will get near full Mastery Value and you have Cthun and Void Eruption. Prune this action for Entropic Rift Builds." );

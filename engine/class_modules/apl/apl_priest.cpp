...
void shadow( player_t* p )
{
  ...
  main->add_action( "devouring_plague,target_if=max:target.time_to_die*(dot.devouring_plague.remains<=gcd.max|variable.dr_force_prio|!talent.distorted_reality&variable.me_force_prio),if=insanity.deficit<=35&talent.distorted_reality|buff.mind_devourer.up&cooldown.mind_blast.up&(cooldown.void_eruption.remains>=3*gcd.max|!talent.void_eruption)&talent.mind_devourer|buff.entropic_rift.up|buff.voidform.up&talent.perfected_form&talent.void_eruption", "Use Devouring Plague to maximize uptime. Short circuit if you are capping on Insanity within 35 With Distorted Reality can maintain more than one at a time in multi-target." );
  ...
  cds->add_action( "void_eruption,if=(pet.fiend.active&cooldown.fiend.remains>=4|!talent.mindbender&!cooldown.fiend.up|active_enemies>2&!talent.inescapable_torment)&(cooldown.mind_blast.charges=0|time>15)", "Make sure Mindbender is active before popping Void Eruption and dump charges of Mind Blast before casting" );
  ...
  filler->add_action( "call_action_list,name=empowered_filler,if=dot.devouring_plague.remains>action.mind_spike.cast_time|!talent.mind_spike", "Consume empowered fillers" );
...
void shadow_ptr( player_t* p )
{
  ...
  trinkets->add_action( "use_item,use_off_gcd=1,name=flarendos_pilot_light,if=gcd.remains>0" );
